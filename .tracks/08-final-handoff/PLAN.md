...
| T08-02 | Deliver `scripts/verify-track-08.sh` and rehearse its clean-clone Python 3.12/locked-`uv` configure, migration, bootstrap/run, full test, OpenAPI/docs, and health workflow. | Smith / implementation | T08-01 | In progress | The final clean-clone automation/orchestrator receipt has exact commands, resolved versions, migration state, service logs, dependency-ordered upstream harness receipts, deterministic unit/integration/contract results, OpenAPI/docs and health selectors, and verified teardown. |
| T08-03 | Reconcile all assessment and accepted-extension requirements with actual results, defect severity, and evidence locations. | Primary engineering thread | T08-01, T08-02 | In progress | Live matrix and reader evidence are updated; complete final reconciliation waits for the clean-clone harness. |
| T08-04 | Author and fresh-reader review root README and supporting reader documentation from delivered facts. | Smith / implementation | T08-02, T08-03 | In progress | Reader package is present and fresh-reader corrections are incorporated; final agreement awaits clean-clone/runtime evidence. |
| T08-05 | Prepare evidence-bounded AI-assisted-work disclosure, walkthrough/demo, and reviewer handoff materials. | Primary engineering thread | T08-03, T08-04 | In progress | Disclosure, walkthrough, and handoff are present; final review waits on T08-03/T08-04 closure. |
...
| T08-07 | Establish the mandatory-green bonus preflight, exact acceptance contracts, isolation order, and rollback/stop conditions for all bonus work. | Primary engineering thread | T08-03, T08-04, T08-05, T08-06 | In progress | ADR-007 and the 2026-08-09 preflight establish the contracts, but formal closure waits for its declared dependencies. |
| T08-08 | Implement all bonus work in isolated green checkpoints: deterministic seed data, Docker setup, rate limiting, and cursor pagination; then rerun the mandatory/full final harness. | Smith / implementation | T08-07 | In progress | Checkpoints are present at `126fadb`, `7d2ea97`/`006d9b1`, `0cdf2a6`, and `cee847f`; focused/full development evidence exists, but the combined clean-clone final-harness receipt is pending. |
