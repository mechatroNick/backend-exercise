...
The delivered branch includes deterministic development seeding (`7d2ea97`, with seed safety tests `006d9b1`), a one-worker Docker delivery path (`126fadb`), local rate limiting (`0cdf2a6`), and authenticated cursor pagination (`cee847f`). ADR-007 records the rate-limit and cursor contracts. The pre-revival clean-source Track 08 harness passed at `ff32511e9cc0e2df8d7681e2c16b3dddb579faae`; its historical evidence is in [the superseded report](../.tracks/08-final-handoff/TEST-REPORT.md).
...
The prior Track 08 clean-source receipt is historical evidence for the pre-revival
scope. Track 08 is reopened and **In progress** until a fresh receipt covers the exact
Track 07 harness, private projection migrations/runtime, unchanged public API, Docker,
hygiene, and verified cleanup.
